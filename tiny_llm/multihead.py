...
if __name__ == "__main__":
    ...
    print()
    print("  What one head CANNOT do, precisely: its weights sum to 1, so")
    print("  attention is a zero-sum allocation. One unit to hand out, per")
    print("  position, total.")
    print()
    print("  Suppose predicting the next word needs BOTH 'who acted' (king) and")
    print("  'what they did' (ruled). A single head has to choose:")
    print()
    print("        0.90 on king and 0.90 on ruled   ->  impossible, that is 1.8")
    print("        0.50 on king and 0.50 on ruled   ->  the best it can manage")
    print()
    print("  So it compromises, and both signals arrive at half strength:")
    print()
    king_value  = torch.tensor([1.0, 0.0])
    ruled_value = torch.tensor([0.0, 1.0])
    print(f"        one head, forced to split:  {[round(v, 2) for v in (0.5 * king_value + 0.5 * ruled_value).tolist()]}   both diluted")
    print()
    print("  Two heads have TWO independent units of attention:")
    print()
    print(f"        head 0, 0.9 on king:        {[round(v, 2) for v in (0.9 * king_value + 0.1 * ruled_value).tolist()]}")
    print(f"        head 1, 0.9 on ruled:       {[round(v, 2) for v in (0.1 * king_value + 0.9 * ruled_value).tolist()]}")
    print()
    print("  Both relationships followed at full strength, side by side, and")
    print("  W_O then decides how much of each the answer should carry. That is")
    print("  the whole reason for more than one head - not more capacity, but")
    print("  more than one place to spend a fixed budget of attention.")
    print()
